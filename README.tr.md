...
**📖 Kılavuz:** [Geliştirici / BYOK kurulumu](docs/INSTALL_BYOK.md) — son kullanıcı (setup.exe) dökümanı [voxislive.com](https://voxislive.com)'da.
...
**Açık kaynak derlemesinin ağ yüzeyi.** Donmuş (frozen) bir geliştirici derlemesi `OFFICIAL` işaretçisi taşımaz; bu yüzden BYOK'a düşer ve **kendi başına hiçbir giden çağrı yapmaz**: kayıt, giriş, doğrulama, kota, sunucu oturum-anahtarı alma, kullanım heartbeat'i ve tüm telemetri devre dışı bırakılır ya da yerel sahte (mock) yanıtlara sabitlenir. Dokunduğu tek ağ, kendi anahtarınızın açtığı Gemini Live WebSocket'idir. Otomatik güncelleme kontrolü yalnızca donmuş derlemelerde çalışır ve varsayılan olarak boş olan `update_check_url` değerine bağlıdır. Genel depo, bir sürüm-hijyen kapısıyla (`scripts/check_release_hygiene.py`, CI'ye ve bir pre-push kancasına bağlı) kapalı-çekirdek yollarından ve canlı sırlardan arındırılmış tutulur.
...
**Kalite ön ayarları** modele gönderilen sürekli akışı şekillendiren yerel VAD geçidine eşlenir. `max_savings` ("Saver"), akışa geçit uygular — yalnızca konuşma gönderilir, sessizlik boşlukları atılır — böylece daha az faturalanan dakika kullanılır. Resmi derleme üç kullanıcı dostu seçenek sunar (**Smooth** = `balanced`, **Fast** = `turbo`, **Saver** = `max_savings`); geliştirici derlemesi tüm ön ayar listesini açar (`max_quality`, `balanced`, `max_savings`, `turbo`).
...
**Arayüz dilleri** (uygulama arabirimi) yalnızca **Türkçe ve İngilizce'dir** — `ui_language` ile ayarlanır. **Çeviri hedef dilleri** (modelin *neye* çevireceği) bundan bağımsızdır ve şunları kapsar: `tr, en, de, fr, es, it, pt, ru, ar, ja, ko, zh-Hans` (`target_language_incoming` / `target_language_outgoing` ile ayarlanır).
...
SaaS arka ucu (`backend/auth-core/`, Go + PocketBase, `voxislive.com` üzerinde Caddy arkasında) oturum başına anahtarlar verir ve kullanımı kaydeder; açık kaynak derleme ona asla bağlanmaz.
